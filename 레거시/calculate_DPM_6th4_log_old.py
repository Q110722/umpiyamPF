...
def cal_DPM_6th4_log(plot_graph, Total_STAT_ARRAY, NET_STAT, ring, VI_level, deal_time_ref, delay):
...
#-------------------------------------------------------------------------------------------------------------------------------
            ...
            if Condition_ARRAY[6] + 0.5*1000 <= t%(120*1000) <= Condition_ARRAY[6] + 0.5*1000 + 20*1000:
                if Relic_Unbound_Delay <= 0 and Relic_Unbound_Stack > 0:
                    ...
                    Relic_Unbound_times_used += 4
                    Relic_Unbound_attacks += 32
                ...
                if Condition_ARRAY[16] <= t%(360*1000) <= Condition_ARRAY[16] + (30+Server_Lag)*1000:
                    if Ancient_Fury_Delay <= 0:
                        ...
                        if Cool == 1:
                            ...
                            Ancient_Fury_Delay = 12000 - iteration_time
                        elif Cool == 0:
            ...
            if Condition_ARRAY[14] + (40 + Server_Lag)*1000 <= t%(120*1000) < Condition_ARRAY[14] + 120*1000:
                if Raven_Delay <= 0:
                    Raven = 2.65*(390)*(100+Damage2+Boss_Damage2)*(135+Critical_Damage2)*(100-Boss_Guard*(1-VMatrix_Ignore_Guard/100))*((100+Attack_Ratio2)*Real_Attack+Attack_lumi)*(DEX2+STR/4)*FINAL_ATTACK
                    Raven_times_used += 1
                    Raven_attacks += 1

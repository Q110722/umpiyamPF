...
def cal_DPM_6th4_restran(plot_graph, Total_STAT_ARRAY, NET_STAT, ring, VI_level, deal_time_ref, delay):
    ...
    # 재사용 
    Ability_Prob = int(Total_STAT_ARRAY[27]) # 10, 20
    ...
    # Link Skill List (Not shown in stat)
    Boss_Damage += 11 + 9 + 12 + 9 + 8.5
    Ignore_Guard = Ignore_Guard + (100 - Ignore_Guard)*0.09

    # 4th Curse Transition
    # Transition VI
    if VI_level[8] > 0:
        Stack = 6 + VI_level[8]//10
    elif VI_level[8] == 0:
        Stack = 5
    Critical_Damage += 4*Stack
    ...
    # 길축, 몬파레드, 유힘, MVP, 우르스, 붕뿌
    Real_Attack += 30 + 30 + 30 + 30 + 30 + 30
    
    # 6차 극딜 빌드
    Condition_ARRAY1 = np.zeros(17)
    Condition_ARRAY1[14] = 0
    Condition_ARRAY1[0] = 0
    Condition_ARRAY1[1] = 540
    Condition_ARRAY1[2] = 1680
    Condition_ARRAY1[3] = 2220
    Condition_ARRAY1[4] = 3120 # Seed Ring
    
    Condition_ARRAY1[7] = 3440 # Barrier
    
    Condition_ARRAY1[5] = 3520 # Evolve_tempest
    Condition_ARRAY1[16] = 3720 # Forsaken (delay 6540 ms)
    
    Condition_ARRAY1[8] = 10260 # Ultimate
    Condition_ARRAY1[13] = 10380 # Relic_evolution
    Condition_ARRAY1[6] = 10730 # Unbound
                                # 5th Bind
    Condition_ARRAY1[9] = 11330
    Condition_ARRAY1[10] = 13720 + 10000
    
    Condition_ARRAY1[11] = 3520
    Condition_ARRAY1[12] = 9000 # 검토 필요!
    
    # 5차 극딜 빌드
    Condition_ARRAY2 = np.zeros(17)
    Condition_ARRAY2[14] = 0
    Condition_ARRAY2[0] = 0
    Condition_ARRAY2[1] = 540
    Condition_ARRAY2[2] = 1680
    Condition_ARRAY2[3] = 2220
    Condition_ARRAY2[4] = 3120 # Seed Ring
    
    Condition_ARRAY2[15] = 3440 # Bind
    
    Condition_ARRAY2[7] = 3540 # Barrier
    Condition_ARRAY2[5] = 3620 # Evolve_tempest
    Condition_ARRAY2[6] = 3920 # Unbound
    
    Condition_ARRAY2[13] = 4640 # Relic_evolution
    Condition_ARRAY2[8] = 4720 # Ultimate

    Condition_ARRAY2[9] = 5980
    Condition_ARRAY2[10] = 13540
    
    Condition_ARRAY2[11] = 3520
    Condition_ARRAY2[12] = 9000 # 검토 필요!
...
#-----------시드링 설정 구간--------------------------------------------------------------------------------------------------------------------
            ...
            # 리레+웨펖 반복, 메용2는 오리진, 리레에 몰빵
            if ring == 1:
                # 일단 메용2 먼저 쓰고
                if Condition_ARRAY[1] <= t%(360*1000) <= Condition_ARRAY[1] + (60 + Server_Lag)*1000 or Condition_ARRAY[1] + 240*1000 <= t%(720*1000) <= Condition_ARRAY[1] + 240*1000 + (60 + Server_Lag)*1000 or Condition_ARRAY[1] + 480*1000 <= t%(720*1000) <= Condition_ARRAY[1] + 480*1000 + (60 + Server_Lag)*1000:
                    Damage2 += 20
                    DEX2 = MY2_DEX
            ...
            # 컨티링만 사용, 메용2는 3개중 2개 꼴로 쿨 도는대로 계속 사용
            if ring == 2:
                # Continuous
                if Condition_ARRAY[1] <= t%(360*1000) <= Condition_ARRAY[1] + (60 + Server_Lag)*1000 or Condition_ARRAY[1] + 120*1000 <= t%(360*1000) <= Condition_ARRAY[1] + 120*1000 + (60 + Server_Lag)*1000:
                    DEX2 = MY2_DEX
                    Damage2 += 20
...
#-------------------------------------------------------------------------------------------------------------------------------
            ...
            if Condition_ARRAY[7] <= t%(120*1000) <= Condition_ARRAY[7] + 25*1000:
                if Obsidian_Barrier_Delay <= 0 and Obsidian_Barrier_Stack > 0:
                    Obsidian_Barrier = VI_level_factor[2]*(1040*4)*(100+Damage2+Boss_Damage2+Ancient_Enchant_Boss_Damage+Enchant_Damage)*(135+Critical_Damage2)*(100-Boss_Guard*(1-(Ignore_Guard2+(100-Ignore_Guard2)*Ancient_Enchant_Ignore_Guard/100)/100))*((100+Attack_Ratio2)*Real_Attack+Attack_lumi)*(DEX2+STR/4)*Ancient_Enchant_Final_Damage*FINAL_ATTACK
            ...
            # Deal Cycle when No-buff and Extreme Deal
            if Condition_ARRAY[9] <= t%(120*1000) < 120*1000:
                ...
                if Resonance_Delay <= 0 and Discharge_Used == 1 and VI_level[7] == 0:
                    Resonance_Num = 1 + Ability_Prob/100
                    Resonance = Resonance_Num*(Cardinal_Ratio2+np.sqrt(Cardinal_Ratio2))/2*(895*6*2.2*(1.1**Stack))*(100+Damage2+Boss_Damage2+Ancient_Enchant_Boss_Damage)*(135+Critical_Damage2)*(100-Boss_Guard*(1-(VMatrix_Ignore_Guard+(100-VMatrix_Ignore_Guard)*Ancient_Enchant_Ignore_Guard/100)/100))*((100+Attack_Ratio2)*Real_Attack+Attack_lumi)*(DEX2+STR/4)*Ancient_Enchant_Final_Damage*FINAL_ATTACK
                    Resonance_Delay = (15*Cool_R-Cool)*1000 - iteration_time
                    Resonance_stack -= 1
                    #씹힘 구현
                    Cardinal_between_Delay = 480
                ...
                if Cardinal_Blast_Delay <= 0 and Active == 1 and VI_level[5] > 0:
                    ...
                    Cardinal_Blast_Delay = 480 - iteration_time
               ...
               # Cardinal Discharge VI
                if Cardinal_Discharge_Delay <= 0 and Active == 1 and Cardinal_between_Delay <= 0 and VI_level[6] > 0:
                    ...
                    Cardinal_Discharge_Delay = 480 - iteration_time
            ...
            # Raven (Delay Example = 2*1000 ms)
            if Condition_ARRAY[14] + (40*Buff_Duration + Server_Lag)*1000 <= t%(120*1000) < Condition_ARRAY[14] + 120*1000:
                if Raven_Delay <= 0:
                    Raven = 2.8*(390)*(100+Damage2+Boss_Damage2)*(135+Critical_Damage2)*(100-Boss_Guard*(1-VMatrix_Ignore_Guard/100))*((100+Attack_Ratio2)*Real_Attack+Attack_lumi)*(DEX2+STR/4)*FINAL_ATTACK
                    Raven_Delay = 2*1000 - iteration_time
                else:
                    Raven = 0
            ...
            # Evolve
            if Condition_ARRAY[14] <= t%(120*1000) < Condition_ARRAY[14] + (40*Buff_Duration + Server_Lag)*1000:
                if Evolve_Delay <= 0:
                    ...
                    Evolve_Delay = 3*1000 - iteration_time
                else:
                    Evolve = 0    
        ...
        # 2D array로 통합
        Deal = np.vstack((Deal_Cardinal_Blast, Deal_Cardinal_Discharge, Deal_Cardinal_Transition, Deal_Additional_Blast, Deal_Additional_Discharge, Deal_Resonance, Deal_Triple_Impact, Deal_Raven, Deal_Guided_Arrow, Deal_Ultimate_Blast, Deal_Evolve_Tempest, Deal_Obsidian_Barrier, Deal_Relic_Unbound, Deal_Evolve, Deal_Forsaken, Deal_Additional_Blast_Curse_Arrow,Deal_Forsaken_Arrow,Deal_Ancient_Fury, Deal_test_graph))
        
        # 최종 딜량 보정 (*무기상수*직업보정상수*랩차보정*속성보정*포스보정*숙련도보정%10^12)
        #for i in range(len(Deal)):
        #    for j in range(len(Deal[i])):
        #        Deal[i][j] *= (1.3 * 1 * 1.2 * (50 + 5/2)/100 * 1.5 * (1 + 0.86)/2) / (10**12)
        
        # 시간, 딜량, 크리인 시간 데이터 return
        return Total_Time, Deal, CRein_time_end
    
    # 딜 계산 함수 실행
    time, Deal, CRein_time_end  = calculate_deal(RT_ratio, Server_Lag, HyperSkill, Damage, Boss_Damage, Ignore_Guard, Critical_Prob, Critical_Damage, DEX_with_MY, MY2_DEX, Weapon_DEX, Attack_Ratio, Real_Attack, Buff_Duration, Restraint_level, deal_time)
    ...
    skill_list = ['Deal_Cardinal_Blast', 'Deal_Cardinal_Discharge', 'Deal_Cardinal_Transition', 'Deal_Additional_Blast', 'Deal_Additional_Discharge', 'Deal_Resonance', 'Deal_Triple_Impact', 'Deal_Raven', 'Deal_Guided_Arrow', 'Deal_Ultimate_Blast', 'Deal_Evolve_Tempest', 'Deal_Obsidian_Barrier', 'Deal_Relic_Unbound', 'Deal_Evolve', 'Deal_Forsaken', 'Deal_Additional_Blast_Curse_Arrow','Deal_Forsaken_Arrow','Deal_Ancient_Fury', 'Deal_test_graph']
    #for i in range(0,len(Deal), 1):
    #    print(skill_list[i], end=': ')
    #    print(Deal_Cumulative[i]/sum*100)
...
# Plot graph ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if plot_graph == 2:
        ...
        axs[0].bar(time_int,Deal_Int[18,:],width=0.6, color = 'black')
    ...
    if Deviation == 1:
            # Deal Deviation
            Deal_Int_Test = Deal_Int[0,:]+Deal_Int[1,:]+Deal_Int[3,:]+Deal_Int[4,:]+Deal_Int[5,:]+Deal_Int[6,:]+Deal_Int[7,:]+Deal_Int[8,:]+Deal_Int[9,:]+Deal_Int[10,:]+Deal_Int[11,:]+Deal_Int[12,:]+Deal_Int[13,:]+Deal_Int[14,:]+Deal_Int[15,:]+Deal_Int[16,:]+Deal_Int[17,:]

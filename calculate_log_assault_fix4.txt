...
def cal_log_assault_fix4(plot_graph, Total_STAT_ARRAY, NET_STAT, ring, VI_level, deal_time_ref, delay):
    ...
    def calculate_deal(RT_ratio, Server_Lag, HyperSkill, Damage, Boss_Damage, Ignore_Guard, Critical_Prob, Critical_Damage, DEX_with_MY, MY2_DEX, Weapon_DEX, Attack_Ratio, Real_Attack, Buff_Duration, Restraint_level,deal_time):
        ...
        Blast_stack = Blast_stack2
        Discharge_stack = Discharge_stack2
...
#-------------------------------------------------------------------------------------------------------------------------------
            ...
            if Condition_ARRAY[9] <= t%(120*1000) < 120*1000:
                ...
                # 포세이큰 버프 없을 시 블디콤레 사이클 유지 
                if Forsaken_buff == 0:        
                    # Assault and Resonance VI
                    if VI_level[8] > 0:
                        if Blast_stack >= Blast_stack2 and Discharge_stack >= Discharge_stack2:

                            Assault = Resonance_Num*2.2*(774+36*VI_level[8])*10*(100+Damage2+Boss_Damage2+Ancient_Enchant_Boss_Damage+Enchant_Damage)*(135+Critical_Damage2)*(100-Boss_Guard*(1-(VMatrix_Ignore_Guard+(100-VMatrix_Ignore_Guard)*Ancient_Enchant_Ignore_Guard/100)/100))*((100+Attack_Ratio2)*Real_Attack+Attack_lumi)*(DEX2+STR/4)*Ancient_Enchant_Final_Damage*FINAL_ATTACK
                            Assault += Resonance_Num*2.2*(696+32*VI_level[8])*7*(100+Damage2+Boss_Damage2+Ancient_Enchant_Boss_Damage+Enchant_Damage)*(135+Critical_Damage2)*(100-Boss_Guard*(1-(VMatrix_Ignore_Guard+(100-VMatrix_Ignore_Guard)*Ancient_Enchant_Ignore_Guard/100)/100))*((100+Attack_Ratio2)*Real_Attack+Attack_lumi)*(DEX2+STR/4)*Ancient_Enchant_Final_Damage*FINAL_ATTACK
                            Assault_times_used += 2*Resonance_Num
                            Assault_attacks += 17*Resonance_Num
                            Assault_between_delay = (600 + 150)*Resonance_Num # 블 또는 디 이후 어썰트 발동까지 딜레이 반영
                            Material_stack += 1*Resonance_Num
                            Ancient_Fury_Delay += 1*1000*Resonance_Num
                    ...
                    # Cardinal Blast VI
                    if Cardinal_Blast_Delay <= 0 and Active == 1 and VI_level[5] > 0 and Assault_between_delay <= 0:
                        Cardinal_Blast = Cardinal_Ratio2*((660+11*VI_level[5])*2.2*Cardinal_Attack)*(100+Damage2+Boss_Damage2+Cardinal_Damage)*(135+Critical_Damage2)*(100-Boss_Guard*(1-VMatrix_Ignore_Guard/100))*((100+Attack_Ratio2)*Real_Attack+Attack_lumi)*(DEX2+STR/4)*FINAL_ATTACK
                        Cardinal_Blast_Delay = 460 - iteration_time
                        Cardinal_Blast_times_used += 1
                        Cardinal_Blast_attacks += 6
                   ...
                   # Cardinal Discharge VI
                    if Cardinal_Discharge_Delay <= 0 and Active == 1 and Cardinal_between_Delay <= 0 and VI_level[6] > 0 and Assault_between_delay <= 0:
                        # x2 attack if there exists a boss
                        Cardinal_Discharge = Cardinal_Ratio2*2*((325+5*VI_level[6])*2.2*Cardinal_Attack)*(100+Damage2+Boss_Damage2+Cardinal_Damage)*(135+Critical_Damage2)*(100-Boss_Guard*(1-VMatrix_Ignore_Guard/100))*((100+Attack_Ratio2)*Real_Attack+Attack_lumi)*(DEX2+STR/4)*FINAL_ATTACK
                        Cardinal_Discharge_Delay = 460 - iteration_time
                        Cardinal_Discharge_times_used += 2
                   ...
                   # Cardinal Discharge
                    ...
                    if Additional_forbidden == 1 and Active_Additional_Discharge == 1:
                        Active_Additional_Discharge = 0
                        Additional_forbidden = 0         
            ...
            if ring != 2:
                if  80*1000 < t%(120*1000) < (80+6)*1000:
                    ...
                    if Resonance != 0:
                        Resonance = 0
                        Resonance_times_used -= 1*Resonance_Num
                        Resonance_attacks -= 6*Resonance_Num
                    if Resonance2 != 0:
                        Resonance2 = 0
                        Resonance_times_used2 -= 1*Resonance_Num
                        Resonance_attacks2 -= 10*Resonance_Num
                    if Material != 0:
                        Material = 0
        ...
        Skill_log_times.append(Assault_times_used)
        ...
        Skill_log_attacks.append(Assault_attacks)
                        
        # 2D array로 통합
        Deal = np.vstack((Deal_Cardinal_Blast, Deal_Cardinal_Discharge, Deal_Cardinal_Transition, Deal_Additional_Blast, Deal_Additional_Discharge, Deal_Resonance, Deal_Triple_Impact, Deal_Raven, Deal_Guided_Arrow, Deal_Ultimate_Blast, Deal_Evolve_Tempest, Deal_Obsidian_Barrier, Deal_Relic_Unbound, Deal_Evolve, Deal_Forsaken, Deal_Additional_Blast_Curse_Arrow,Deal_Forsaken_Arrow,Deal_Ancient_Fury, Deal_Material, Deal_Resonance2, Deal_Assault, Deal_test_graph))
    ...
    with open('log.txt','w') as file:
        ...
        file.write(f"총 딜량: {(sum/10**12):.9f}\n\n")
        
        file.write('이볼브 템페스트\n')
        file.write(f"{Deal_Cumulative[10]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[10]:.9f}\n")
        file.write(f"{Skill_log_times[0]}\n")
        file.write(f"{Skill_log_attacks[0]}\n\n")
        
        file.write('렐릭 언바운드\n')
        file.write(f"{Deal_Cumulative[12]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[12]:.9f}\n")
        file.write(f"{Skill_log_times[1]}\n")
        file.write(f"{Skill_log_attacks[1]}\n\n")
        
        file.write('옵시디언 배리어\n')
        file.write(f"{Deal_Cumulative[11]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[11]:.9f}\n")
        file.write(f"{Skill_log_times[2]}\n")
        file.write(f"{Skill_log_attacks[2]}\n\n")
        
        file.write('얼티밋 블래스트\n')
        file.write(f"{Deal_Cumulative[9]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[9]:.9f}\n")
        file.write(f"{Skill_log_times[3]}\n")
        file.write(f"{Skill_log_attacks[3]}\n\n")
        
        file.write('포세이큰 렐릭\n')
        file.write(f"{(Deal_Cumulative[14]+Deal_Cumulative[16]+Deal_Cumulative[17])/sum*100:.9f}\n")
        file.write(f"{(Deal_Cumulative[14]+Deal_Cumulative[16]+Deal_Cumulative[17]):.9f}\n")
        file.write(f"{Skill_log_times[4]+Skill_log_times[8]+Skill_log_times[9]}\n")
        file.write(f"{Skill_log_attacks[4]+Skill_log_attacks[8]+Skill_log_attacks[9]}\n\n")
        
        file.write('엣지 오브 레조넌스\n')
        file.write(f"{Deal_Cumulative[5]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[5]:.9f}\n")
        file.write(f"{Skill_log_times[5]}\n")
        file.write(f"{Skill_log_attacks[5]}\n\n")
        
        file.write('에인션트 임팩트\n')
        file.write(f"{Deal_Cumulative[19]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[19]:.9f}\n")
        file.write(f"{Skill_log_times[17]}\n")
        file.write(f"{Skill_log_attacks[17]}\n\n")
        
        file.write('콤보 어썰트\n')
        file.write(f"{Deal_Cumulative[20]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[20]:.9f}\n")
        file.write(f"{Skill_log_times[18]}\n")
        file.write(f"{Skill_log_attacks[18]}\n\n")
        
        file.write('카디널 블래스트\n')
        file.write(f"{Deal_Cumulative[0]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[0]:.9f}\n")
        file.write(f"{Skill_log_times[6]}\n")
        file.write(f"{Skill_log_attacks[6]}\n\n")
        
        file.write('카디널 디스차지\n')
        file.write(f"{Deal_Cumulative[1]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[1]:.9f}\n")
        file.write(f"{Skill_log_times[7]}\n")
        file.write(f"{Skill_log_attacks[7]}\n\n")
        
        file.write('에디셔널 디스차지\n')
        file.write(f"{Deal_Cumulative[4]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[4]:.9f}\n")
        file.write(f"{Skill_log_times[10]}\n")
        file.write(f"{Skill_log_attacks[10]}\n\n")
        
        file.write('에디셔널 블래스트\n')
        file.write(f"{(Deal_Cumulative[3]+Deal_Cumulative[15])/sum*100:.9f}\n")
        file.write(f"{(Deal_Cumulative[3]+Deal_Cumulative[15]):.9f}\n")
        file.write(f"{Skill_log_times[11]+Skill_log_times[12]}\n")
        file.write(f"{Skill_log_attacks[11]+Skill_log_attacks[12]}\n\n")
        
        file.write('레이븐\n')
        file.write(f"{Deal_Cumulative[7]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[7]:.9f}\n")
        file.write(f"{Skill_log_times[13]}\n")
        file.write(f"{Skill_log_attacks[13]}\n\n")
        
        file.write('이볼브\n')
        file.write(f"{Deal_Cumulative[13]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[13]:.9f}\n")
        file.write(f"{Skill_log_times[14]}\n")
        file.write(f"{Skill_log_attacks[14]}\n\n")
        
        file.write('가이디드 애로우\n')
        file.write(f"{Deal_Cumulative[8]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[8]:.9f}\n")
        file.write(f"{Skill_log_times[15]}\n")
        file.write(f"{Skill_log_attacks[15]}\n\n")
        
        file.write('렐릭 마테리얼라이즈\n')
        file.write(f"{Deal_Cumulative[18]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[18]:.9f}\n")
        file.write(f"{Skill_log_times[16]}\n")
        file.write(f"{Skill_log_attacks[16]}\n\n")
        
        file.write('참고------------------\n\n')
        
        file.write('에디셔널 블래스트 (기본화살)\n')
        file.write(f"{Deal_Cumulative[3]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[3]:.9f}\n")
        file.write(f"{Skill_log_times[11]}\n")
        file.write(f"{Skill_log_attacks[11]}\n\n")
        
        file.write('에디셔널 블래스트 (저주화살)\n')
        file.write(f"{Deal_Cumulative[15]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[15]:.9f}\n")
        file.write(f"{Skill_log_times[12]}\n")
        file.write(f"{Skill_log_attacks[12]}\n\n")
        
        file.write('포세이큰 렐릭 (컷신)\n')
        file.write(f"{Deal_Cumulative[14]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[14]:.9f}\n")
        file.write(f"{Skill_log_times[4]}\n")
        file.write(f"{Skill_log_attacks[4]}\n\n")
        
        file.write('포세이큰 렐릭 (마법화살)\n')
        file.write(f"{Deal_Cumulative[16]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[16]:.9f}\n")
        file.write(f"{Skill_log_times[8]}\n")
        file.write(f"{Skill_log_attacks[8]}\n\n")
        
        file.write('포세이큰 렐릭 (고대의 분노)\n')
        file.write(f"{Deal_Cumulative[17]/sum*100:.9f}\n")
        file.write(f"{Deal_Cumulative[17]:.9f}\n")
        file.write(f"{Skill_log_times[9]}\n")
        file.write(f"{Skill_log_attacks[9]}\n\n")
...
# Plot graph ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if plot_graph == 2:
        ...
        axs[0].bar(time_int,Deal_Int[20,:],width=0.6, color = 'lime', bottom = Deal_Int[0,:]+Deal_Int[1,:]+Deal_Int[3,:]+Deal_Int[4,:]+Deal_Int[5,:]+Deal_Int[6,:]+Deal_Int[7,:]+Deal_Int[8,:]+Deal_Int[9,:]+Deal_Int[10,:]+Deal_Int[11,:]+Deal_Int[12,:]+Deal_Int[13,:]+Deal_Int[14,:]+Deal_Int[15,:]+Deal_Int[16,:]+Deal_Int[17,:]+Deal_Int[18,:]+Deal_Int[19,:])

        # Test_graph
        axs[0].bar(time_int,Deal_Int[21,:],width=0.6, color = 'white')

...
def cal_log_new_rev12(plot_graph, Total_STAT_ARRAY, NET_STAT, ring, VI_level, deal_time_ref):
    ...
    def calculate_deal(stats, deal_time):
        Total_Time = []
        timeline_log = []
        state = log_array()
        ...
        def Ascent_D(pct, bonus_boss, bonus_ign):
            return calc_Ascent_D(pct, bonus_boss, bonus_ign, stats, Boss_Guard)
        #def Ascent_D(pct, bonus_boss, bonus_ign):
            #return calc_Ascent_D(pct, bonus_boss, bonus_ign, Total_STAT_ARRAY, NET_STAT, Boss_Guard)
        ...
        for t in range(0*1000,deal_time*1000,iteration_time):
            ...
            Penetration = 0
            if VI_level[9] > 0:
                elapsed_pen = t - 100*1000 #시전 시간 지정
                if 0 <= elapsed_pen <= 6600:
                    Skill_Lock = 1 if elapsed_pen < 6600 else 0
                    pen_schedule = {0: 6, 2210: 6, 4410: 6, 1500: 19, 3700: 19, 5910: 19}
                    if elapsed_pen in pen_schedule:
                        ...
                        Penetration = Ascent_D(pct_penetration, pen_boss, pen_ign)
 ...
 #-----------버프류 설정 구간--------------------------------------------------------------------------------------------------------------------          
            ...
            if Condition_ARRAY[4] <= (t % (T40+stats["Cool"]*1000))  <  Condition_ARRAY[4] + 20*1000 + svr_lag: Damage2 += 17 # 카인 링크
            if Condition_ARRAY[4] <= (t % (T20+stats["Cool"]*1000))  <  Condition_ARRAY[4] + 10*1000 + svr_lag: Damage2 += 18 # 모도 링크
            if Condition_ARRAY[2] <= (t % T120) < Condition_ARRAY[2] + 30*1000 + svr_lag: #크리인
                Critical_Damage2 += Critical_Prob2 / 2       
                # 첫번째 크리인은 따로 저장
                if t < Condition_ARRAY[2] + 30*1000 + svr_lag:                 
                    CRein_time_end = t
...
#-------------------------------------------------------------------------------------------------------------------------------            
            ...
            # Ultimate Blast
            #Ultimate_Blast = 0
            #if Condition_ARRAY[8] + 1.8*1000 <= t % T120 <= Condition_ARRAY[8] + 16.8*1000:
            #    if Ultimate_Blast_Stack > 0:
            #        Ultimate_Blast = D(VI_level_factor[3] * 6 * 1500 * 15, 100, Ancient_Enchant_Boss_Damage, 0, Ancient_Enchant_Final_Damage + 1)           
            #        Ultimate_Blast_Stack -= 1
            ...
            if cycle_time >= Condition_ARRAY[9]:
                Cardinal_Ratio2 = Cardinal_Ratio if cycle_time >= Condition_ARRAY[10] else 1
                ...
                if state["Cardinal_Blast"]["delay"] <= 0 and Active == 1 and Assault_between_delay <= 0:
                    if VI_level[5] > 0:
                        pct = Cardinal_Ratio2 * (569 + 6 * VI_level[5]) * 2.2 * Cardinal_Attack
                    else:
                        pct = Cardinal_Ratio2 * (520 + 11 * stats["Ability_Passive"]) * 2.2 * Cardinal_Attack
                ...
                if state["Cardinal_Discharge"]["delay"] <= 0 and Active == 1 and Cardinal_between_Delay <= 0 and Assault_between_delay <= 0:
                    if VI_level[6] > 0:
                        pct = Cardinal_Ratio2 * 2 * (299 + 2 * VI_level[6]) * 2.2 * Cardinal_Attack
                    else:
                        pct = Cardinal_Ratio2 * 2 * (273 + 4 * stats["Ability_Passive"]) * 2.2 * Cardinal_Attack
                ...
                if Forsaken_buff == 1:
                    if Condition_ARRAY[16] <= t % T360 <= Condition_ARRAY[16] + (30 + stats["Server_Lag"]) * 1000:
                        if Active_Additional_Blast == 1 or Active_Additional_Discharge == 1:
                            pct = Cardinal_Ratio2 * (810 + 27 * VI_level[4]) * 5
                    ...
                    if Condition_ARRAY[16] <= t % T360 <= Condition_ARRAY[16] + (30+stats["Server_Lag"])*1000:
                        if state["Ancient_Fury"]["delay"] <= 0:
                            pct_fury = Cardinal_Ratio2 * (650 + 21 * VI_level[4]) * 45
                ...
                if Active_Additional_Discharge == 1 and Active == 1:
                    if VI_level[6] > 0:
                        ...
                        pct = Cardinal_Ratio2 * ratio * (base_val + base_val * 0.7 * (arrow_num - 1)) * 2.2
                    else:
                        arrow_num = 4 if Active_Relic_Evolution == 1 else 3
                        ratio = Additional_Ratio / 100
                        pct = Cardinal_Ratio2 * ratio * (166 + stats["Ability_Passive"]) * 3 * 2.2 * arrow_num
                ...
                if Active_Additional_Blast == 1 and Active == 1:
                    if VI_level[5] > 0:
                        ...
                        ratio = Additional_Ratio / 100
                        pct = Cardinal_Ratio2 * ratio * (221 + stats["Ability_Passive"]) * 3 * 2.2 * arrow_num
            ...
            if lvl11 > 0:
                ...
                evolve_start = Condition_ARRAY[5] + 10000
            ...
            elif elapsed_com <= 48300 and elapsed_com % 2100 == 0:
            ...
            elif elapsed_com <= 49980 and elapsed_com % 8330 == 0: 
            ...
            skill_order = [
                ...
                "Ancient_Fury", "Material", "Resonance2", "Assault", "test_graph", 
            ...
            active_skills = [key for key, value in zip(skill_order, skill_values) if value > 0]
            if active_skills:
                timeline_log.append((t, active_skills))
        
        log_order = [
            "Evolve_Tempest", "Relic_Unbound", "Obsidian_Barrier", "Ultimate_Blast",
            "Forsaken_Relic", "Resonance", "Cardinal_Blast", "Cardinal_Discharge",
            "Forsaken_Arrow", "Ancient_Fury", "Additional_Discharge", "Additional_Blast",
            "Additional_Blast_Curse_Arrow", "Raven", "Evolve", "Guided_Arrow",
            "Material", "Resonance2", "Assault", "Hecate", "Styx", "Phlegethon", "Penetration",
            "CoS", "SiM", "Evolve_Orbit"
        ]
        
        Skill_log_times = [state[name]["times_used"] for name in log_order]
        Skill_log_attacks = [state[name]["attacks"] for name in log_order]
        ...
        Deal = np.vstack([state[name]["damage"] for name in damage_order])
        deal_multiplier = 4 * (1.3 * 1 * 1.2 * (50 + 5/2)/100 * 1.25 * (1 + 0.86)/2) / (10**12)
        Deal *= deal_multiplier
        
        return Total_Time, Deal, CRein_time_end, Skill_log_times, Skill_log_attacks, damage_order, state, timeline_log
    
    time, Deal, CRein_time_end, Skill_log_times, Skill_log_attacks, damage_order, state, timeline_log = calculate_deal(stats, deal_time)
    Deal_Cumulative = np.sum(Deal, axis=1)
    Deal_Int = Deal.reshape(Deal.shape[0], deal_time, 100).sum(axis=2)
    time_int = np.arange(1, deal_time + 1)
    Deal_Int_C = np.sum(Deal_Int, axis=0)
    Deal_Int_CC = np.cumsum(Deal_Int_C)    
    sum = Deal_Cumulative.sum()
    write_skill_log(sum, Deal_Cumulative, damage_order, state, 'log.txt')
    write_timeline_log(timeline_log, 'timeline_log.txt')
    
    jo = int(sum // 10**12)
    euk = int((sum % 10**12) // 10**8)
    man = int((sum % 10**8) // 10**4)
    print(f"{deal_time/60:.1f}분 딜량 = {jo}조 {euk}억 {man}만")
    draw_graph(plot_graph, deal_time, time_int, Deal_Int, Deal_Int_C, Deal_Int_CC, CRein_time_end, Cardinal_Ratio, stats, Deviation=0)
